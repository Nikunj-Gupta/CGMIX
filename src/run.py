...
def evaluate_sequential_compare(args, runner):
    ...
    runner.mac.greedy_action_selector.onoff_configamount = args.iterative_onoff_configamount
    compare_onoff_configamount = 2 ** args.mixing_embed_dim
    #label1 = r"Iterative optimization ($n_{max}=4$)"
    label1 = r"Iterative optimization ($n_{max}=" + str(runner.mac.greedy_action_selector.onoff_configamount) + r"$)"
    #label2 = r"NL-CG (embed=3, iterative, $n_{max}=4$)"
    label2 = r"NL-CG (embed=3, iterative, $n_{max}=" + str(runner.mac.greedy_action_selector.onoff_configamount) + r"$)"


    envsavepath = args.env.capitalize()
    ...
    for _ in range(100):    
        ...
        batches = runner.run_compare(test_mode=True, trial_num = trial_num, t_assigned=t_assigned)
    ...
    ax.set_title(label2 + "(" + envsavepath + ")",titlefontdict,pad=20)
    fig.subplots_adjust(left=0.15)
    fig.savefig('plot_results/opt_eff/' + envsavepath + '-qgt_qtot-cam.png')
    ...
    ax.scatter(qvals_compare, qvals, label=label1, edgecolor=None, c='k', s=50,marker='s')
    ax.plot(np.arange(-1e3,1e3,1),np.arange(-1e3,1e3,1),'b--', label='Line y=x',zorder=2)#红色虚线
    fontdict1 = {"size":15,"color":"k",}
    ...
    fig.subplots_adjust(left=0.15)
    fig.savefig('plot_results/opt_eff/' + envsavepath + '-qtot_diffselection-cam.png')
    ...
    ax.plot(np.arange(-1e3,1e3,1),np.arange(-1e3,1e3,1),'b--', label='Line y=x',zorder=2)#红色虚线
    fontdict1 = {"size":15,"color":"k",}
    ax.set_xlabel(label2, fontdict=fontdict1)
    ...
    fig.subplots_adjust(left=0.15)
    fig.savefig('plot_results/opt_eff/' + envsavepath + '-qgt-cam.png')
    
    #=============================

    fig, ax = plt.subplots(figsize=(6,4.5))
    ax.plot(np.arange(0,5,0.1),np.arange(0,5,0.1),'b--', label='Line y=x')#红色虚线
    ax.scatter(times_compare, times, label=label1, edgecolor=None, c='k', s=10,marker='s')
    ...
    fig.savefig('plot_results/opt_eff/'+ envsavepath + '-time-m4-cam.png')
